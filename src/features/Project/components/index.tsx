...
export default function ProjectModal({ project, onClose }: ProjectModalProps) {
  const thumbnail = `/images/project/${project.id}/thumbnail.${project.thumbnailFormat || 'webp'}`;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="relative w-full max-w-6xl max-h-[90vh] overflow-y-auto bg-neutral-900 text-white rounded-xl p-4 md:p-8">
        ...
        <div className="flex flex-col gap-6 mt-8">
          ...
          <div className="flex flex-col md:flex-row gap-6">
            ...
            <div className="flex-1">
              <section className="bg-[#5d5e67] rounded-[10px] p-6">
                <h2 className="text-lg font-bold mb-4">프로젝트 요약</h2>
                <div className="space-y-4 text-base leading-6">
                  <div>
                    <p className="font-semibold mb-1">프로젝트명</p>
                    <p>{project.title}</p>
                  </div>
                  <div>
                    <p className="font-semibold mb-1">한 줄 소개</p>
                    <p>{project.subTitle}</p>
                  </div>
                  <div>
                    <p className="font-semibold mb-1">팀 구성</p>
                    <p className="whitespace-pre-wrap">{project.teamMembers || '미등록'}</p>
                  </div>
                  <div>
                    <p className="font-semibold mb-1">프로젝트 기간</p>
                    <p>{project.period || '미등록'}</p>
                  </div>
                  <div>
                    <p className="font-semibold mb-1">사용 기술 스택</p>
          ...
          <section className="bg-[#2f3038] rounded-[10px] p-6">
            ...
            <div className="h-[200px] overflow-auto whitespace-pre-wrap">
              {project.description}
            </div>
          </section>
        </div>
